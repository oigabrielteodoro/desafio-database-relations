...
class ProductsRepository implements IProductsRepository {
  ...
  public async updateQuantity(
    ...
    const updatedProducts = findAllProductsById.map(product => {
      ...
      if (quantity <= 0) {
        throw new AppError('The product is not available');
      }
    ...
    const saved = await this.ormRepository.save(updatedProducts);
